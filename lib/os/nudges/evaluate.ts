...
async function evalInvoice(supabase: ServiceClient, rule: EngagementRule) {
  const cutoff = new Date(Date.now() - rule.threshold * DAY_MS).toISOString();
  // The date test runs in Postgres alongside the status test. Filtering it
  // here instead meant fetching every open invoice on every run and throwing
  // most of them away — and PostgREST caps a response at 1000 rows, so past
  // that the evaluator would quietly stop seeing the oldest ones, which are
  // precisely the overdue ones this rule exists to find.
